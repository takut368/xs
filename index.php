...
$seiseiFile = __DIR__ . '/data/seisei.json';
    ...
    <head>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // ログインフォームの表示制御
                const userForm = document.querySelector('.link-form');
                const toggleLinkFormButton = document.getElementById('toggleLinkForm');
                        ...
                        <?php foreach ($seiseiData[$currentUser] as $id => $link): ?>
                            <div class="link-item">
                                ...
                                <div class="link-actions">
                                    <button onclick="window.open('<?php echo htmlspecialchars('https://' . $_SERVER['HTTP_HOST'] . '/' . $id, ENT_QUOTES, 'UTF-8'); ?>', '_blank')">表示</button>
                                    <button onclick="window.location.href='edit_link.php?id=<?php echo urlencode($id); ?>'">編集</button>
                                    <button onclick="if(confirm('本当に削除しますか？')){ window.location.href='delete_link.php?id=<?php echo urlencode($id); ?>' }">削除</button>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
